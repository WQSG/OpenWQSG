...
IMPLEMENT_DYNAMIC(C统计字频, CDialog)
...
void C统计字频::统计一句字频_过滤控制符(WCHAR* TXT)
{
	while(*TXT)
	{
		if(W_KZF_L == *TXT)
...
void C统计字频::统计一句字频_纯文本(WCHAR* TXT)
{
	while(*TXT)
	{
		if(0x20 > *TXT)
...
BEGIN_MESSAGE_MAP(C统计字频, CDialog)
...
void C统计字频::OnBnClickedButtonTbl()
{
	...
	CWQSGFileDialog fopendlg(TRUE,NULL,NULL,OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,_T("码表文件(*.TBL,*.TXT)|*.TBL;*.TXT||"));
...
void C统计字频::OnBnClickedButton2()
{
	...
	fopendlg.m_ofn.lpstrTitle = _T("选择要统计的文本...");
	fopendlg.m_ofn.lpstrInitialDir = strPath;
...
void C统计字频::OnEnChangeEdit3()
{
	...
	// TODO:  在此添加控件通知处理程序代码
	UpdateData();
	编辑框验证十六进制文本(W_第一个编码,(CEdit*)GetDlgItem(IDC_EDIT_TBL_3),(CWnd*)this,0);
}
...
void C统计字频::OnBnClickedButtonTbl3()
{
	...
	CWQSGFileDialog fopendlg(FALSE,_T("TBL"),NULL,OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,_T("码表文件(*.TBL)|*.TBL||"));
	fopendlg.m_ofn.lpstrTitle = _T("码表保存到...");
	fopendlg.m_ofn.lpstrInitialDir = strPath;
...
void C统计字频::OnBnClickedButton3()
{
	...
	CWQSGFileDialog fopendlg(TRUE,NULL,NULL,OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,_T("编码表(*.TBL,*.TXT)|*.TBL;*.TXT||"));
	fopendlg.m_ofn.lpstrTitle = _T("选择一个编码表表...");
	fopendlg.m_ofn.lpstrInitialDir = strPath;
...
BOOL C统计字频::编码表编码(::CWQSG_File * TXT_File)
{
	...
	while(*字符串)
	{
		...
		{
			delete[]tmp2;
			if(IDOK != 对话框.show( 一行 , L"等号左边不是有效的十六进制文本,此行作废,要继续吗?" , MB_YESNO) )
			{
				delete[]一行;
				return FALSE;
			}
